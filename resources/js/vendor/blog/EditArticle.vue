					...
					<div class="add-listing-section">
						...
						<div class="row with-forms">
							...
							<div class="col-md-12">
								...
								<select class="chosen-select-no-single" v-model="article.blog_category_id">
									<option value="null">Избери категория</option>
									<option v-for="c in categories" :key="c.id" :value="c.id"
											:selected="article.blog_category_id == c.id ? 'true' : 'false'"> {{ c.name
										}}
									</option>
								</select>
	...
	export default {
		...
		methods: {
			...
			async saveArticle() {
				...
				if (!this.article.description || !this.article.title || !this.article.blog_category_id) {
				...
				try {
					...
					this.$refs.mytoast.s(
						"Статията бе редактирана успешно!"
					);
					setTimeout(() => {
				...
				try {
					...
					for (var i in this.article.article_galleries) {
						if (this.article.article_galleries[i].main) {
							...
							var file = {size: 123, name: this.article.article_galleries[i].name, type: "image/jpg"};
