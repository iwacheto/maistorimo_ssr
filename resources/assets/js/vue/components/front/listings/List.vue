      ...
      <div class="row project_content">
        ...
        >
          <div class="sidebar">
            ...
            <span v-if="showTags&& selectTag.length>0" class="tag_info">{{ selectTag.length }} избран<span v-if="selectTag.length>1">и</span> таг<span v-if="selectTag.length>1">а</span></span>
            <!-- More Search Options -->
            ...
            <div class="checkboxes one-in-row margin-bottom-15" v-if="showThirtyTags">
              <div v-for="tag in ThirtyTags" :key="tag.id">
                <input :id="tag.tag" type="checkbox" :name="tag.tag" :value="tag.tag" />
                <label :for="tag.tag">{{tag.tag | lowercase}}</label>
              </div>
            </div>
            <!-- Checkboxes / End -->
